...
def train(model, data_loader, logger, include_knowledge, args):
    ...
    for epoch in range(args.epoch_n):
        ...
        with tqdm(
            ...
            for i, data in enumerate(data_loader["train"]):
                ...
                if args.loss_function == "nll":
                    output_pair = 1 - output
                    output = torch.cat((output_pair, output), 1)
                    score = score.long()
                    loss = loss_function(torch.log(output), score)
                else:
        ...
        save_snapshot(
            model, f"checkpoint/{args.dataset}/{args.model}/model_epoch{str(epoch+1)}"
        )
...
if __name__ == "__main__":
    ...
    data_loader = {
        ...
        "val": generate_dataloader(args.val_set, args, include_knowledge, True),
    }
    logger.info(
